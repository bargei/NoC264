...
uint32_t send_luma_intra_packet_3(int size, frame *f, int bx, int by, int blkIdx_test, uint64_t intra_request, int identifier)
{
	...
	if((size == 4) && (blkIdx_test)){
		// write data 1
		set_luma_all(f, bx, by, 3, -1, 1);

		// write data 2
		set_luma_all(f, bx, by, 3, -1, 2);
	}else{
		// write data 1
		set_luma_x(f, bx, by, 3, -1, 1);

		// write data 2
		set_luma_x(f, bx, by, 7, -1, 2);
	}
	...
	if(size == 16){
		// write data 3
		set_luma_x(f, bx, by, 11, -1, 3);

		// write   data 4
		set_luma_single(f, bx, by, 15, -1, 4);

		// write   data 7
		set_luma_y(f, bx, by, -1, 8, 7);

		// write   data 8
		set_luma_y(f, bx, by, -1, 12, 8);
	}
