...
int main(int argc, char *argv[]) {
  ...
  //initialize the profiling counters
  intra_counter    = 0;
  inter_counter    = 0;
  deblock_counter  = 0;
  flits_rx_counter = 0;
  ...
  //already tested... removed for convenience
  //printf("CPU 0 sending packet\n");
  //send_flit(7, 0, 0, 1);
  //buffers_loop(1);
  //printf("rxd packet on CPU 0!\n");
...
//    sd_fwrite(f->L,f->Lheight,out);
//    sd_fwrite(f->C[0],f->Cheight,out);
//    sd_fwrite(f->C[1],f->Cheight,out);
    //if(frame_no>=100) break;
  }
  //stop = time(0);
  //diff = stop - start;
  //printf("video decoded (%d sec)\nclosing file (may be slow...)\n", -1);

  //fclose(out);
  //alt_up_sd_card_fclose(out);
  h264_close();

  printf("DONE!\n");
  printf("intra_counter    = %llu\n", intra_counter   );
  printf("inter_counter    = %llu\n", inter_counter   );
  printf("deblock_counter  = %llu\n", deblock_counter );
  printf("flits_rx_counter = %llu\n", flits_rx_counter);
  printf("flits_tx_counter = %llu\n", flits_tx_counter);
  printf("iqit_counter     = %llu\n", iqit_counter    );
  printf("iqit_dc_counter  = %llu\n", iqit_dc_counter );
  printf("calvc_counter    = %llu\n", calvc_counter   );
  printf("nal_counter      = %llu\n", nal_counter     );
  printf("other_counter    = %llu\n", other_counter   );
  return 0;
}
